...
```bash
pip install torch numpy pandas stable-baselines3 gym tqdm scikit-learn streamlit
```
...
6. **Interactive User Interface with Streamlit**:
   The project includes a Streamlit-based web interface (`app.py`) for configuring and running the counterfactual generation algorithm interactively.

    **Features**:

    - **Method Selection**: Choose between generating counterfactuals for all samples or multiple counterfactuals for a specific sample.
    - **Dataset Loading**: Specify a dataset path or upload a CSV file.
    - **Parameter Configuration**: Set training mode, timesteps, feature constraints, MCTS options, and more through an intuitive interface.
    - **Dynamic Constraints**: Add constraints for specific features with tailored options (e.g., "increase", "decrease", "fixed" for numerical features; "fixed", "change" for categorical features).
    - **Real-Time Logging**: View training and generation progress in a dedicated log panel.
    - **Results Display**: Visualize original and counterfactual DataFrames and metrics (coverage, distance, implausibility, sparsity, actionability, diversity) in the UI.

    **How to Use**:

    1. Ensure Streamlit is installed (see Requirements).
    2. Run the Streamlit app:
        ```bash
        streamlit run app.py
        ```
    3. Open the provided URL (e.g., `http://localhost:8501`) in a web browser.
    4. Select the generation method from the sidebar.
    5. Configure parameters in the main section:
        - Load a dataset (path or upload).
        - Set training mode, timesteps, indices/sample index, constraints, max steps, MCTS options, and save path.
    6. Click the "Run" button (enabled only when all required fields are valid) to generate counterfactuals.
    7. View logs, DataFrames, and metrics in the interface.

    **Notes**:

    - The "Run" button is disabled if the dataset is invalid or the sample index is out of range.
    - Logs are displayed in real-time, showing training and generation progress.
    - The Streamlit app does not require modifications to `PPO_env.py` or `PPO_train.py`.
